      ...
      <div className="max-w-6xl mx-auto px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <button onClick={() => exportCSV(applications, 'enrolment.csv',
              ...
              {applications.map((app: any, i: number) => (
                <tr key={app.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${app.status === 'approved' ? 'bg-green-100 text-green-700' : app.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      {app.status}
                    </span>
                  </td>
                  <td className="px-6 py-3">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="font-semibold text-gray-800">Exam Results Report</h3>
            <button onClick={() => exportCSV(examRegs, 'exam-results.csv',
              ['Candidate', 'Exam', 'Course', 'Result'],
              (r: any) => `${r.profiles?.full_name},${r.exam_sessions?.name},${r.exam_sessions?.courses?.name},${r.result || 'pending'}`)}
              className="text-xs px-3 py-1.5 rounded-lg text-white font-medium" style={{ background: '#c4a020' }}>
              Export CSV
            </button>
          </div>
            ...
            <tbody>
              {examRegs.map((reg: any, i: number) => (
                <tr key={reg.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${reg.result === 'pass' ? 'bg-green-100 text-green-700' : reg.result === 'fail' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>
                      {reg.result || 'pending'}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-sm text-gray-600">{reg.appeal_status || '—'}</td>
                </tr>
              ))}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <button onClick={() => exportCSV(portfolios, 'portfolio.csv',
              ['Candidate', 'Course', 'Title', 'Type', 'Domain', 'Status'],
              (p: any) => `${p.profiles?.full_name},${p.courses?.name},${p.title},${p.entry_type || 'Clinical Case'},${p.competency_domain},${p.status}`)}
              className="text-xs px-3 py-1.5 rounded-lg text-white font-medium" style={{ background: '#c4a020' }}>
              Export CSV
            </button>
          </div>
            ...
            <tbody>
              {portfolios.map((p: any, i: number) => (
                <tr key={p.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-3 text-sm text-gray-600">{p.entry_type || 'Clinical Case'}</td>
                  <td className="px-6 py-3 text-sm text-gray-600">{p.competency_domain}</td>
                  <td className="px-6 py-3">
              ...
              {portfolios.length === 0 && <tr><td colSpan={6} className="text-center py-6 text-gray-400 text-sm">No portfolio entries yet</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
